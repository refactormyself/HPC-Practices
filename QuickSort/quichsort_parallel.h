...
inline void quicksort_parallel(int arr[], int left, int right)
{
	if (left < right)
	{
		int pivot = choose_pivot_(left, right); //may be random instead of the middle
...
#pragma omp parallel sections
		{
#pragma omp section
		quicksort_parallel(arr, left, j - 1);
#pragma omp section
		quicksort_parallel(arr, j + 1, right);
		}
*/

#pragma omp parallel //private(left, j)
		quicksort_parallel(arr, left, partition_pt - 1);
#pragma omp parallel //private(right, j)
